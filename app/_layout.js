...
function InnerApp() {
    ...
    useEffect(() => {
        ...
        const shouldRTL = Localization.getLocales()?.[0]?.languageScriptCode === "Arab" ||
    ...
    const uiMode = (contrast === "high"
        ? baseMode === "dark"
            ...
            : "highContrastLight"
        : baseMode);
    return (_jsx(ClerkProvider, { tokenCache: tokenCache, publishableKey: publishableKey, children: _jsx(ConvexProviderWithClerk, { client: convex, useAuth: useAuth, children: _jsx(ClerkLoaded, { children: _jsx(SafeAreaProvider, { children: _jsx(UIThemeProvider, { mode: uiMode, children: _jsx(NavigationThemeProvider, { value: effectiveTheme, children: _jsxs(Stack, { children: [_jsx(Stack.Screen, { name: "(tabs)", options: { headerShown: false } }), _jsx(Stack.Screen, { name: "(auth)", options: { headerShown: false } }), _jsx(Stack.Screen, { name: "+not-found" })] }) }) }) }) }) }) }));
}
export default function RootLayout() {
    return (_jsx(SettingsProvider, { children: _jsx(InnerApp, {}) }));
}
